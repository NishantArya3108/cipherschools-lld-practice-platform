...
export default function HistoryPage() {
  ...
  useEffect(() => {
    getAttempts().then(setAttempts).finally(() => setLoading(false));
  }, []);
    ...
    <div className="container page">
      <div className="page-heading">
        <div>
          ...
          <p>Review what changed between attempts.</p>
        </div>
      </div>
      ...
      {attempts.length === 0 ? (
        <div className="empty-state">
          ...
          <p>Choose a problem and start your first design.</p>
          <Link className="button primary" to="/problems">Browse problems</Link>
        </div>
      ) : (
        <div className="history-list">
          {attempts.map((attempt) => (
            <article className="history-card" key={attempt.id}>
              <div>
                <span className="muted">{new Date(attempt.updatedAt).toLocaleString()}</span>
                <h2>{attempt.problemTitle}</h2>
